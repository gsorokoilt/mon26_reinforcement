...
#read committee array
project[:steps].each do |name|
#add PERSON key/value to each hash
name[:member] = project[:committee].first#name[:member] assigns values from |name| to kv pair as value
project[:committee].rotate!#rotate runs through list of committee array objects
end

p project

#assign each name in the committee to a variable  ?



# add committee member name to each step
# steps per member should be equal
#member names should only appear once in code
